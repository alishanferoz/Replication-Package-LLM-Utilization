...
OUTPUT_SIG_FILE = "Significance test analysis_Output.xlsx"
...
ACCESS_BENEFIT_STEM = "How much benefit do you get from the integration of LLM"
...
def run_significance_analysis(df_full):
    ...
    for sheet_name, stem in SHEET_MAPPING.items():
        ...
        # Determine target benefit stem based on context
        target_ben_stem = None
        if sheet_name in ["Requirements", "Software Design", "Development", "Testing"]:
            target_ben_stem = DISTRIBUTE_BENEFIT_STEM
        elif sheet_name == "Methods":
            target_ben_stem = ACCESS_BENEFIT_STEM

        if target_ben_stem:
            ben_cols = [c for c in df_full.columns if target_ben_stem.lower() in c.lower()]
            ben_rows = []
            for bc in ben_cols:
                b_task = extract_subtask(bc)
                if b_task in df_table.index:
...
def get_sig_label(task, category, sig_data, data_type):
    ...
    row_sum, col_sum = df.loc[task].sum(), df[category].sum()
    grand_total = df.sum().sum()
...
def plot_combined_stacked(df, matched_data, sig_info, title, filename, top_label_text="Usage"):
    """Base plotting function for stacked usage/benefit charts."""
        ...
        for i, cat in enumerate(scale):
            ...
            for j, rect in enumerate(bars):
                ...
                if w > 5:
                    ...
                    # Vertical stack: Percentage -> Count -> Significance
                    main_label = f"{w:.0f}%\n({counts[j]})"
                    if marker: main_label += "\n "  # Placeholder for spacing

                    ax.text(x_mid, y_mid, main_label, ha='center', va='center',
                            color=text_color, fontsize=16, fontweight='bold')

                    if marker:
                        ax.text(x_mid, y_mid, f"\n\n{marker}", ha='center', va='center',
    ...
    for y in y_idx:
        ax.text(-1, y - 0.1, top_label_text, ha='right', va='center', fontsize=14, fontstyle='italic', color='#444')
        ax.text(-1, y + 0.1, "Benefit", ha='right', va='center', fontsize=14, fontstyle='italic', color='#444')

    leg_title = f"{top_label_text} Frequency"
    leg1 = ax.legend(handles=[Patch(facecolor=c, label=l) for c, l in zip(COLORS_USAGE, FREQ_SCALE_USAGE)],
                     title=leg_title, loc='upper left', bbox_to_anchor=(1.02, 1.0), fontsize=14)
    ax.add_artist(leg1)
...
def plot_access_method_chart(df, matched_data, sig_info, filename):
    """Specific function for Access Method chart to handle 'Access' naming."""
    plot_combined_stacked(df, matched_data, sig_info, "", filename, top_label_text="Access")
...
def main():
    ...
    # Step 1: Run Significance Analysis
    sig_cache = run_significance_analysis(df)

    # Step 2: SDLC Task Charts
    ...
    for s_key, stem, fname in sdlc_configs:
    ...
    # Step 3: Access Method Chart
    acc_cols = [c for c in df.columns if "How often do you use the following methods" in c]
    acc_ben_cols = [c for c in df.columns if ACCESS_BENEFIT_STEM in c]
    acc_matched = []
    for ac in acc_cols:
    ...
    if acc_matched:
        ...
        # Calling specific function for Access Methods to handle 'Access' naming
        plot_access_method_chart(df, acc_matched, sig_cache.get("Methods"), "chart_F_access_combined.png")
